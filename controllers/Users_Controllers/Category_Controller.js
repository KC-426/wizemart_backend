...
const createCategory = async(req,res)=>{
    ...
    try{
        const findExistingCategory = await Category_Schema.find({main_category_slug:req.body.main_category_slug?.toLowerCase(),category_slug:req.body.category_slug?.toLowerCase()})
        ...
        const findMainCategory = await Category_Schema.find({main_category_name:req.body.mainCategory})
        ...
        if(findMainCategory[0]?.category_name ){
           ...
           return res.status(200).send({status:true,message:"success"})
            ...
            const updateCategory = await Category_Schema.findByIdAndUpdate(findMainCategory[0]?._id,
                {
                    $set:{
                            main_category_name:req.body.mainCategory?.toLowerCase(),
                            main_category_slug:req.body.main_category_slug?.toLowerCase(),
...
const createMainCategory = async(req,res)=>{
    try{
        const findExistingCategory = await Category_Schema.find({main_category_name:req.body.mainCategory?.toLowerCase()})
        if(findExistingCategory.length > 0){
...
const editCategory = async(req,res)=>{
    ...
    try{
        ...
        const findOldCate = await Category_Schema.findById(categoryId)
        if(!findOldCate) return res.status(404).send("please provide a category id")
        const updateProductMainCate = await Products_Schema.updateMany({product_category:findOldCate?.category_name},
            {
                $set:{
                    product_category:req?.body?.category_name?.toLowerCase(),
                    product_category_slug:req?.body?.category_slug?.toLowerCase(),
                }
        })
        console.log("updateProductMainCate-->",updateProductMainCate)
        const findCategory = await Category_Schema.findByIdAndUpdate(categoryId,{$set:req.body})
        if(!findCategory){
...
const updateMainCategory=async(req,res)=>{
    ...
    try{
        if(oldMainCategory){
            const find = await Category_Schema.find({main_category_name:oldMainCategory})
            console.log("FIND___",find)
            if(find?.length > 0){
                const updateAll = await Category_Schema.updateMany({main_category_name:oldMainCategory},
                    {
                        $set:{
                    ...
                    const updateProductMainCate = await Products_Schema.updateMany({product_main_category:oldMainCategory},
                        {
                            $set:{
...
const deleteImage = async(req,res)=>{
    ...
    try{
        if(oldMainCategory){
            const find = await Category_Schema.find({main_category_name:oldMainCategory});
            if(find?.length > 0){
                const removeImage = await Category_Schema.updateMany({main_category_name:oldMainCategory},
                        {$set:{
                            main_category_image:{}
                        }}
                    )
        ...
        if(oldCategory){
            const findCategory = await Category_Schema.find({category_name:oldCategory});
            console.log(findCategory);
            if(findCategory?.length > 0 ){
                const removeImage = await Category_Schema.updateMany({category_name:oldCategory},
                    {
                        $set:{
                            category_image:{}
                        }
        ...
        if(oldSubId && oldSubCategory){
            ...
            const findSubCategroy = await Category_Schema.find({_id:oldSubId,"subcategory.name":oldSubCategory});
            ...
            if(findSubCategroy?.length > 0 ){
               ...
               await result?.map((value,index)=>{
                ...
                const updateSubCategory = await Category_Schema.findByIdAndUpdate({_id:oldSubId},{$set:{subcategory:result}})
...
// GET ALL CATEGORY / CATEGORY FILTER / SEARCH FOR CATEGROY
const getAllCategory = async(req,res)=>{
    ...
    if(main_category != 'all' ){
        result = await Category_Schema.find({main_category_name:main_category})
                ...
                .skip((page - 1) * limit);
        console.log(result)
        count = await Category_Schema.find({main_category_name:main_category}).count()
        return res.status(200).send({all_categories:result,countCategory:count,pages:Math.ceil(count / limit),categoryForFilter:categoryForFilter})
    ...
    // =========== category search ==========
    if(searchValue){
        result = await Category_Schema.find({main_category_name:{$regex:searchRegex}})
                .sort({createdAt:-1})
                .limit(limit)
                .skip((page - 1) * limit);
        count = await Category_Schema.find({main_category_name:{$regex:searchRegex}}).count()
       if(!result.length ){
           result = await Category_Schema.find({category_name:{$regex:searchRegex}})
                    .sort({createdAt:-1})
                    .limit(limit)
                    .skip((page - 1) * limit);
           count = await Category_Schema.find({category_name:{$regex:searchRegex}}).count()
           if(!result.length){
               result = await Category_Schema.find({"subcategory.name":{$regex:searchRegex}})
                        .sort({createdAt:-1})
                        .limit(limit)
                        .skip((page - 1) * limit);
               count = await Category_Schema.find({"subcategory.name":{$regex:searchRegex}}).count()
           }
       }
    ...
    // ========== all categories ============
        result = await Category_Schema.find({})
                ...
                .limit(limit)
                .skip((page - 1) * limit);
        count = await Category_Schema.find({}).count()
        res.status(200).send({all_categories:result,countCategory:count,pages:Math.ceil(count / limit),categoryForFilter:categoryForFilter})
    // ========== all categories ============
...
const deleteCategory=async(req,res)=>{
    ...
    try {
        if(req.body?.data?.length){
            const findImagesToDelete= await Category_Schema.find({
                _id:{
                    $in:req.body?.data
                }
        ...
        return res.status(200).send({message:"image delete success",image_need_to_delete:findImagesToDelete,status:true})
